...
日期：2026-06-10
状态：已实施完成。两处与原设计的偏离（均经用户确认）：
1. 主题改为**默认浅色 + 手动切浅/深，不跟随系统**（原定跟随系统）；
2. 视觉不用系统控件默认样式，而是复刻 Avalonia 版设计稿的设计语言（双主题色板 + 自定义控件样式 + 呼吸/过渡动画，见 `src/macOS/Poco/Views/Theme.swift`）。
`src/Avalonia/` 已删除（git 历史可查）。
